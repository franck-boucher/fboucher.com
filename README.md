...
```sh
flyctl launch --name fboucher-com-7122 --copy-config --no-deploy
```
...
To get the current app URL and IP address. The app URL will be `https://fboucher-com-7122.fly.dev`. You can visit that URL, and the site should be online. That's it. You have deployed your blog built using REMIX!.
...
After buying the domain, we can add a DNS record to point to the domain or create a subdomain and point that to the Fly app URL. We can do that by adding a DNS record using the CNAME option and entering the Fly URL `https://fboucher-com-7122.fly.dev`.
